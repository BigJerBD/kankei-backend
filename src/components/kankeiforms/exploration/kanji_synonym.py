...
class KanjiSynonym(KankeiForm):
    group = "Exploration"
    name = "Get synonym (kanji)"
    tooltip = ""
    ...
    fields = [
        ...
        IntField(
            name="Synonym limit",
            template_name="kanji_limit",
            description="maximum number of path",
            default=200,
            validate=lambda x: 0 <= x <= 1000,
            validate_error_message='"Synonym limit" must be between 0 and 250',
        )
    ]
    ...
    def get_query(cls, **kwargs):
        ...
        if kwargs["english_meaning_search"].choice == "Yes":
            base_query += "OPTIONAL MATCH p0=(k)-[:HasMeaning]-(:English)-[:IsSynonym]-(:Meaning)<-[:HasMeaning]-(:Character) "
            to_collect.append("p0")

        if kwargs["japanese_meaning_search"].choice == "Yes":
            base_query += "OPTIONAL MATCH p1=(k)-[:HasMeaning]-(:Japanese)-[:IsSynonym]->(:Kanji) "
            base_query += "OPTIONAL MATCH p2=(k)-[:IsSynonym]-(:Japanese)<-[:HasMeaning]-(:Kanji) "
            base_query += "OPTIONAL MATCH p3=(k)-[:HasArchaism]-(:Kanji) "
            to_collect.append("p1")
            to_collect.append("p2")
            to_collect.append("p3")

        to_collect = [f"collect({p})[..{kwargs['kanji_limit'].value}]" for p in to_collect]
